...
function onBeforeSendHeaders(details) {
    ...
    // if the request header does not contain cookie return the Request as it is
    if(!utils.headerExists(details, "Cookie")) {
        return { requestHeaders: details.requestHeaders };
    }

    // IF THE CODE IS HERE THIS MEANS THAT THE REQUEST CONTAINS COOKIES
...
function xhRequest(rId) {
    let responseOneData = firstResponseHeaders[rId]; //First Response
    let requestOnedata = xhrData[rId]; //First Request
